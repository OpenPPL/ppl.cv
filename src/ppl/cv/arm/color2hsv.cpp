    ...
    uint8_t* dst)
{
    ...
    for (int32_t k = 0; k < height; k++, srcPtr += src_step, dstPtr += dst_step) {
        ...
        for (i = 0; i <= width - 8; i += 8) {
            ...
            uint16x8_t r = vmovl_u8(v_src.val[0]);
            uint16x8_t g = vmovl_u8(v_src.val[1]);
            uint16x8_t b = vmovl_u8(v_src.val[2]);
...
::ppl::common::RetCode RGB2HSV<uint8_t>(
    ...
    uint8_t* outData)
{
    return rgb_to_hsv_u8<3, 3>(height, width, inWidthStride, inData, outWidthStride, outData);
}
