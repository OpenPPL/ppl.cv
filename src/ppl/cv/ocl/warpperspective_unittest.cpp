...
inline std::string convertToStringWarpPerspective(const Parameters& parameters) {
...
class PplCvOclWarpPerspectiveTest : public ::testing::TestWithParam<Parameters> {
 public:
  PplCvOclWarpPerspectiveTest() {
...
template <typename T, int channels>
bool PplCvOclWarpPerspectiveTest<T, channels>::apply() {
  float scale_coeff;
  if (scale == kHalfSize) {
    scale_coeff = 0.5f;
  }
  ...
  int border_value = 5;
  cv::warpPerspective(src, cv_dst, M, cv::Size(dst_width, dst_height),
  ...
  ppl::cv::ocl::WarpPerspective<T, channels>(queue, src.rows, src.cols,
      ...
      border_value);
  ppl::cv::ocl::WarpPerspective<T, channels>(queue, size.height, size.width,
...
using PplCvOclWarpPerspectiveTest ## T ## channels =                           \
        PplCvOclWarpPerspectiveTest<T, channels>;                              \
TEST_P(PplCvOclWarpPerspectiveTest ## T ## channels, Standard) {               \
  bool identity = this->apply();                                               \
  EXPECT_TRUE(identity);                                                       \
}                                                                              \
                                                                               \
INSTANTIATE_TEST_CASE_P(IsEqual, PplCvOclWarpPerspectiveTest ## T ## channels, \
  ::testing::Combine(                                                          \
