...
void dilate2DU8Kernel0(global const uchar* src, int rows, int cols,
  ...
  for (int i = bottom_y; i <= top_y; i++) {
    ...
    while (index <= top_x3) {
      ...
      if (index >= bottom_x0 && index <= top_x0) {
        result.x = max(result.x, target.x);
      }
      if (index >= bottom_x1 && index <= top_x1) {
        result.y = max(result.y, target.x);
      }
      if (index >= bottom_x2 && index <= top_x2) {
        result.z = max(result.z, target.x);
      }
      if (index >= bottom_x3 && index <= top_x3) {
        result.w = max(result.w, target.x);
      }
      index++;

      if (index >= bottom_x0 && index <= top_x0) {
        result.x = max(result.x, target.y);
      }
      if (index >= bottom_x1 && index <= top_x1) {
        result.y = max(result.y, target.y);
      }
      if (index >= bottom_x2 && index <= top_x2) {
        result.z = max(result.z, target.y);
      }
      if (index >= bottom_x3 && index <= top_x3) {
        result.w = max(result.w, target.y);
      }
      index++;

      if (index >= bottom_x0 && index <= top_x0) {
        result.x = max(result.x, target.z);
      }
      if (index >= bottom_x1 && index <= top_x1) {
        result.y = max(result.y, target.z);
      }
      if (index >= bottom_x2 && index <= top_x2) {
        result.z = max(result.z, target.z);
      }
      if (index >= bottom_x3 && index <= top_x3) {
        result.w = max(result.w, target.z);
      }
      index++;

      if (index >= bottom_x0 && index <= top_x0) {
        result.x = max(result.x, target.w);
      }
      if (index >= bottom_x1 && index <= top_x1) {
        result.y = max(result.y, target.w);
      }
      if (index >= bottom_x2 && index <= top_x2) {
        result.z = max(result.z, target.w);
      }
      if (index >= bottom_x3 && index <= top_x3) {
        result.w = max(result.w, target.w);
      }
      index++;
    }
    data += src_stride;
  ...
  if (border_type == BORDER_CONSTANT) {
    if (constant_border0) {
      result.x = max(result.x, border_value);
    }

    if (constant_border1) {
      result.y = max(result.y, border_value);
    }

    if (constant_border2) {
      result.z = max(result.z, border_value);
    }

    if (constant_border3) {
      result.w = max(result.w, border_value);
    }
  }
...
void dilate2DF32Kernel0(global const float* src, int rows, int cols,
  ...
  if (border_type == BORDER_CONSTANT && constant_border) {
    result = max(result, border_value);
  }
...
__kernel
void dilateColU8Kernel0(global const uchar* src, int rows, int cols,
...
void dilateRowU8Kernel1(global const uchar* src, int rows, int cols,
  ...
  int index = bottom_x0;
  uchar3 target;
  uchar3 result0 = (uchar3)(0, 0, 0);
  uchar3 result1 = (uchar3)(0, 0, 0);
  uchar3 result2 = (uchar3)(0, 0, 0);
  uchar3 result3 = (uchar3)(0, 0, 0);
  global uchar* data = src + element_y * src_stride;
  while (index <= top_x3) {
    target = vload3(index, data);
    if (index >= bottom_x0 && index <= top_x0) {
      result0 = max(result0, target);
    }
    if (index >= bottom_x1 && index <= top_x1) {
      result1 = max(result1, target);
    }
    if (index >= bottom_x2 && index <= top_x2) {
      result2 = max(result2, target);
    }
    if (index >= bottom_x3 && index <= top_x3) {
      result3 = max(result3, target);
    }
    index++;
  }
  ...
  if (border_type == BORDER_CONSTANT) {
    uchar3 borders = (uchar3)(border_value, border_value, border_value);
    if (constant_border0) {
      result0 = max(result0, borders);
    }
    if (constant_border1) {
      result1 = max(result1, borders);
    }
    if (constant_border2) {
      result2 = max(result2, borders);
    }
    if (constant_border3) {
      result3 = max(result3, borders);
    }
  }
...
__kernel
void dilateColU8Kernel1(global const uchar* src, int rows, int cols,
                        int src_stride, int radius_y, global uchar* dst,
                        int dst_stride, enum BorderType border_type,
                        uchar border_value) {
  int element_x = get_global_id(0);
...
void dilateRowU8Kernel2(global const uchar* src, int rows, int cols,
  ...
  int index = bottom_x0;
  uchar4 target;
  uchar4 result0 = (uchar4)(0, 0, 0, 0);
  uchar4 result1 = (uchar4)(0, 0, 0, 0);
  uchar4 result2 = (uchar4)(0, 0, 0, 0);
  uchar4 result3 = (uchar4)(0, 0, 0, 0);
  global uchar* data = src + element_y * src_stride;
  while (index <= top_x3) {
...
__kernel
void dilateColU8Kernel2(global const uchar* src, int rows, int cols,
                        int src_stride, int radius_y, global uchar* dst,
                        int dst_stride, enum BorderType border_type,
                        uchar border_value) {
  int element_x = get_global_id(0);
...
void dilateRowF32Kernel0(global const float* src, int rows, int cols,
  ...
  if (border_type == BORDER_CONSTANT) {
    if (constant_border0) {
      result0 = max(result0, border_value);
    }
    if (constant_border1) {
      result1 = max(result1, border_value);
    }
    if (constant_border2) {
      result2 = max(result2, border_value);
    }
    if (constant_border3) {
      result3 = max(result3, border_value);
    }
  }
...
__kernel
void dilateColF32Kernel0(global const float* src, int rows, int cols,
...
__kernel
void dilateColF32Kernel1(global const float* src, int rows, int cols,
...
__kernel
void dilateColF32Kernel2(global const float* src, int rows, int cols,
                         int src_stride, int radius_y, global float* dst,
                         int dst_stride, enum BorderType border_type,
                         float border_value) {
  int element_x = get_global_id(0);
