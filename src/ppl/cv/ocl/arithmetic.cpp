...
RetCode mlaU8(const cl_mem src0, int rows, int cols, int channels,
              int src0_stride, const cl_mem src1, int src1_stride,
              cl_mem dst, int dst_stride, cl_command_queue queue) {
  PPL_ASSERT(src0 != nullptr);
  PPL_ASSERT(src1 != nullptr);
  ...
  if (src0_stride == columns && src1_stride == columns &&
    ...
    frame_chain->setCompileOptions("-D MLA_U81D");
    runOclKernel(frame_chain, "mlaU8Kernel0", 2, global_size, local_size, src0,
                 columns, src1, dst);
  }
  else {
    columns = cols * channels;
    ...
    frame_chain->setCompileOptions("-D MLA_U82D");
    runOclKernel(frame_chain, "mlaU8Kernel1", 2, global_size, local_size, src0,
                 rows, columns, src0_stride, src1, src1_stride, dst,
                 dst_stride);
  }
...
RetCode mlaF32(const cl_mem src0, int rows, int cols, int channels,
               int src0_stride, const cl_mem src1, int src1_stride,
               cl_mem dst, int dst_stride, cl_command_queue queue) {
  PPL_ASSERT(src0 != nullptr);
  PPL_ASSERT(src1 != nullptr);
  ...
  if ((src0_stride & 7) == 0 && (src1_stride & 7) == 0 &&
      (dst_stride & 7) == 0) {
    cols = divideUp(columns, 2, 1);
    frame_chain->setCompileOptions("-D MLA_F32ALIGNED");
    runOclKernel(frame_chain, "mlaF32Kernel0", 2, global_size, local_size, src0,
                 rows, cols, src0_stride, src1, src1_stride, dst,
                 dst_stride);
  }
  else {
    frame_chain->setCompileOptions("-D MLA_F32UNALIGNED");
    runOclKernel(frame_chain, "mlaF32Kernel1", 2, global_size, local_size, src0,
                 rows, columns, src0_stride, src1, src1_stride, dst,
                 dst_stride);
  }
...
template <>
RetCode Mla<uchar, 1>(cl_command_queue queue,
                      int height,
                      int width,
                      int inWidthStride0,
                      const cl_mem inData0,
                      ...
                      int outWidthStride,
                      cl_mem outData) {
  RetCode code = mlaU8(inData0, height, width, 1, inWidthStride0, inData1,
                       inWidthStride1, outData, outWidthStride, queue);
...
template <>
RetCode Mla<uchar, 3>(cl_command_queue queue,
                      int height,
                      int width,
                      int inWidthStride0,
                      const cl_mem inData0,
                      ...
                      int outWidthStride,
                      cl_mem outData) {
  RetCode code = mlaU8(inData0, height, width, 3, inWidthStride0, inData1,
                       inWidthStride1, outData, outWidthStride, queue);
...
template <>
RetCode Mla<uchar, 4>(cl_command_queue queue,
                      int height,
                      int width,
                      int inWidthStride0,
                      const cl_mem inData0,
                      ...
                      int outWidthStride,
                      cl_mem outData) {
  RetCode code = mlaU8(inData0, height, width, 4, inWidthStride0, inData1,
                       inWidthStride1, outData, outWidthStride, queue);
...
template <>
RetCode Mla<float, 1>(cl_command_queue queue,
                      int height,
                      int width,
                      int inWidthStride0,
                      const cl_mem inData0,
  ...
  inWidthStride1 *= sizeof(float);
  outWidthStride *= sizeof(float);
  RetCode code = mlaF32(inData0, height, width, 1, inWidthStride0, inData1,
                        inWidthStride1, outData, outWidthStride, queue);
...
template <>
RetCode Mla<float, 3>(cl_command_queue queue,
                      int height,
                      int width,
                      int inWidthStride0,
                      const cl_mem inData0,
  ...
  inWidthStride1 *= sizeof(float);
  outWidthStride *= sizeof(float);
  RetCode code = mlaF32(inData0, height, width, 3, inWidthStride0, inData1,
                        inWidthStride1, outData, outWidthStride, queue);
...
template <>
RetCode Mla<float, 4>(cl_command_queue queue,
                      int height,
                      int width,
                      int inWidthStride0,
                      const cl_mem inData0,
  ...
  inWidthStride1 *= sizeof(float);
  outWidthStride *= sizeof(float);
  RetCode code = mlaF32(inData0, height, width, 4, inWidthStride0, inData1,
                        inWidthStride1, outData, outWidthStride, queue);

  return code;
}

RetCode mlsU8(const cl_mem src0, int rows, int cols, int channels,
              int src0_stride, const cl_mem src1, int src1_stride,
              cl_mem dst, int dst_stride, cl_command_queue queue) {
  PPL_ASSERT(src0 != nullptr);
  PPL_ASSERT(src1 != nullptr);
  ...
  if (src0_stride == columns && src1_stride == columns &&
    ...
    frame_chain->setCompileOptions("-D MLS_U81D");
    runOclKernel(frame_chain, "mlsU8Kernel0", 2, global_size, local_size, src0,
                 columns, src1, dst);
  }
  else {
    columns = cols * channels;
    ...
    frame_chain->setCompileOptions("-D MLS_U82D");
    runOclKernel(frame_chain, "mlsU8Kernel1", 2, global_size, local_size, src0,
                 rows, columns, src0_stride, src1, src1_stride, dst,
                 dst_stride);
  }
...
RetCode mlsF32(const cl_mem src0, int rows, int cols, int channels,
               int src0_stride, const cl_mem src1, int src1_stride,
               cl_mem dst, int dst_stride, cl_command_queue queue) {
  PPL_ASSERT(src0 != nullptr);
  PPL_ASSERT(src1 != nullptr);
  ...
  if ((src0_stride & 7) == 0 && (src1_stride & 7) == 0 &&
      (dst_stride & 7) == 0) {
    cols = divideUp(columns, 2, 1);
    frame_chain->setCompileOptions("-D MLS_F32ALIGNED");
    runOclKernel(frame_chain, "mlsF32Kernel0", 2, global_size, local_size, src0,
                 rows, cols, src0_stride, src1, src1_stride, dst,
                 dst_stride);
  }
  else {
    frame_chain->setCompileOptions("-D MLS_F32UNALIGNED");
    runOclKernel(frame_chain, "mlsF32Kernel1", 2, global_size, local_size, src0,
                 rows, columns, src0_stride, src1, src1_stride, dst,
                 dst_stride);
  }
...
template <>
RetCode Mls<uchar, 1>(cl_command_queue queue,
                      int height,
                      int width,
                      int inWidthStride0,
                      const cl_mem inData0,
                      ...
                      int outWidthStride,
                      cl_mem outData) {
  RetCode code = mlsU8(inData0, height, width, 1, inWidthStride0, inData1,
                       inWidthStride1, outData, outWidthStride, queue);
...
template <>
RetCode Mls<uchar, 3>(cl_command_queue queue,
                      int height,
                      int width,
                      int inWidthStride0,
                      const cl_mem inData0,
                      ...
                      int outWidthStride,
                      cl_mem outData) {
  RetCode code = mlsU8(inData0, height, width, 3, inWidthStride0, inData1,
                       inWidthStride1, outData, outWidthStride, queue);
...
template <>
RetCode Mls<uchar, 4>(cl_command_queue queue,
                      int height,
                      int width,
                      int inWidthStride0,
                      const cl_mem inData0,
                      ...
                      int outWidthStride,
                      cl_mem outData) {
  RetCode code = mlsU8(inData0, height, width, 4, inWidthStride0, inData1,
                       inWidthStride1, outData, outWidthStride, queue);
...
template <>
RetCode Mls<float, 1>(cl_command_queue queue,
                      int height,
                      int width,
                      int inWidthStride0,
                      const cl_mem inData0,
  ...
  inWidthStride1 *= sizeof(float);
  outWidthStride *= sizeof(float);
  RetCode code = mlsF32(inData0, height, width, 1, inWidthStride0, inData1,
                        inWidthStride1, outData, outWidthStride, queue);
...
template <>
RetCode Mls<float, 3>(cl_command_queue queue,
                      int height,
                      int width,
                      int inWidthStride0,
                      const cl_mem inData0,
  ...
  inWidthStride1 *= sizeof(float);
  outWidthStride *= sizeof(float);
  RetCode code = mlsF32(inData0, height, width, 3, inWidthStride0, inData1,
                        inWidthStride1, outData, outWidthStride, queue);
...
template <>
RetCode Mls<float, 4>(cl_command_queue queue,
                      int height,
                      int width,
                      int inWidthStride0,
                      const cl_mem inData0,
  ...
  inWidthStride1 *= sizeof(float);
  outWidthStride *= sizeof(float);
  RetCode code = mlsF32(inData0, height, width, 4, inWidthStride0, inData1,
                        inWidthStride1, outData, outWidthStride, queue);
