...
bool PplCvOclMergeToTest<T, channels>::apply() {
  ...
  cl_mem gpu_input2 = clCreateBuffer(context,
                                    CL_MEM_READ_ONLY | CL_MEM_ALLOC_HOST_PTR,
                                    src_bytes1, NULL, &error_code);
  CHECK_ERROR(error_code, clCreateBuffer);

  cl_mem gpu_input3 = clCreateBuffer(context,
                                    CL_MEM_READ_ONLY | CL_MEM_ALLOC_HOST_PTR,
                                    src_bytes1, NULL, &error_code);
  CHECK_ERROR(error_code, clCreateBuffer);
  ...
  error_code = clEnqueueUnmapMemObject(queue, gpu_input0, input0, 0, NULL, NULL);
  CHECK_ERROR(error_code, clEnqueueUnmapMemObject);

  copyMatToArray(src1, input1);
  error_code = clEnqueueUnmapMemObject(queue, gpu_input1, input1, 0, NULL, NULL);
  CHECK_ERROR(error_code, clEnqueueUnmapMemObject);

  copyMatToArray(src2, input2);
  error_code = clEnqueueUnmapMemObject(queue, gpu_input2, input2, 0, NULL, NULL);
  CHECK_ERROR(error_code, clEnqueueUnmapMemObject);

  copyMatToArray(src3, input3);
  error_code = clEnqueueUnmapMemObject(queue, gpu_input3, input3, 0, NULL, NULL);
  CHECK_ERROR(error_code, clEnqueueUnmapMemObject);
  ...
  if (channels == 3) {
    ...
    ppl::cv::ocl::Merge3Channels<T>(queue, size.height, size.width,
        size.width, gpu_input0, gpu_input1, gpu_input2, 
    ...
    ppl::cv::ocl::Merge4Channels<T>(queue, src0.rows, src0.cols,
        ...
        dst.step / sizeof(T), gpu_dst);
    ppl::cv::ocl::Merge4Channels<T>(queue, size.height, size.width,
        size.width, gpu_input0, gpu_input1, gpu_input2, gpu_input3,
        size.width * channels, gpu_output);
  }
