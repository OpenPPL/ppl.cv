...
bool PplCvOclCalchistTest::apply() {
  ...
  CHECK_ERROR(error_code, clCreateBuffer);
  cl_mem gpu_input_mask = clCreateBuffer(context,
                                    CL_MEM_READ_ONLY | CL_MEM_HOST_WRITE_ONLY,
  ...
  uchar* input_mask = (uchar*)clEnqueueMapBuffer(queue, gpu_input_mask, CL_TRUE, CL_MAP_WRITE,
  ...
  error_code = clEnqueueUnmapMemObject(queue, gpu_input_mask, input_mask, 0, NULL, NULL);
  CHECK_ERROR(error_code, clEnqueueUnmapMemObject);
  ...
  if (is_masked == kUnmasked) {
    ...
    ppl::cv::ocl::CalcHist(queue, src.rows, src.cols,
        src.step / sizeof(uchar), gpu_src, gpu_dst, mask0.step / sizeof(uchar), nullptr);
    ...
    ppl::cv::ocl::CalcHist(queue, size.height, size.width,
        size.width, gpu_input, gpu_output, size.width, nullptr);
    output = (int*)clEnqueueMapBuffer(queue, gpu_output, CL_TRUE, CL_MAP_READ,
                                      0, dst_bytes1, 0, NULL, NULL, &error_code);
    CHECK_ERROR(error_code, clEnqueueMapBuffer);
  }
  else {
    cv::calcHist(&src, 1, channel, mask0, cv_dst1, 1, hist_size, ranges,
                 true, false);

    ppl::cv::ocl::CalcHist(queue, src.rows, src.cols,
        src.step / sizeof(uchar), gpu_src, gpu_dst, mask0.step / sizeof(uchar), gpu_mask);
    error_code = clEnqueueReadBuffer(queue, gpu_dst, CL_TRUE, 0, dst_bytes0,
                                    dst.data, 0, NULL, NULL);
    CHECK_ERROR(error_code, clEnqueueReadBuffer);

    ppl::cv::ocl::CalcHist(queue, size.height, size.width,
        size.width, gpu_input, gpu_output, size.width, gpu_input_mask);
    output = (int*)clEnqueueMapBuffer(queue, gpu_output, CL_TRUE, CL_MAP_READ,
                                      0, dst_bytes1, 0, NULL, NULL, &error_code);
    CHECK_ERROR(error_code, clEnqueueMapBuffer);
  }
  ...
  bool identity0 = checkMatricesIdentity<int>((const int*)cv_dst.data, cv_dst.rows,
      cv_dst.cols, cv_dst.channels(), cv_dst.step, (const int*)dst.data, dst.step,
      epsilon);
  bool identity1 = checkMatricesIdentity<int>((const int*)cv_dst.data, cv_dst.rows,
      cv_dst.cols, cv_dst.channels(), cv_dst.step, output,
      256 * sizeof(int), epsilon);
