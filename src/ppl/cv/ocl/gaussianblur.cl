...
#if defined(GAUSSIANBLUR_interpolateReplicateBorderU8C1) ||                    \
    defined(GAUSSIANBLUR_interpolateReplicateBorderF32C1) ||                   \
    defined(GAUSSIANBLUR_interpolateReplicateBorderU8C3) ||                    \
    defined(GAUSSIANBLUR_interpolateReplicateBorderF32C3) ||                   \
    defined(GAUSSIANBLUR_interpolateReplicateBorderU8C4) ||                    \
    defined(GAUSSIANBLUR_interpolateReplicateBorderF32C4) ||                   \
    defined(GAUSSIANBLUR_interpolateReflectBorderU8C1) ||                      \
    defined(GAUSSIANBLUR_interpolateReflectBorderF32C1) ||                     \
    defined(GAUSSIANBLUR_interpolateReflectBorderU8C3) ||                      \
    defined(GAUSSIANBLUR_interpolateReflectBorderF32C3) ||                     \
    defined(GAUSSIANBLUR_interpolateReflectBorderU8C4) ||                      \
    defined(GAUSSIANBLUR_interpolateReflectBorderF32C4) ||                     \
    defined(GAUSSIANBLUR_interpolateReflect101BorderU8C1) ||                   \
    defined(GAUSSIANBLUR_interpolateReflect101BorderF32C1) ||                  \
    defined(GAUSSIANBLUR_interpolateReflect101BorderU8C3) ||                   \
    defined(GAUSSIANBLUR_interpolateReflect101BorderF32C3) ||                  \
    defined(GAUSSIANBLUR_interpolateReflect101BorderU8C4) ||                   \
    defined(GAUSSIANBLUR_interpolateReflect101BorderF32C4) ||                  \
    defined(ALL_KERNELS)
__kernel
...
#define GAUSSIANBLURF32U8C1KERNEL(interpolate)                               \
__kernel                                                                     \
void gaussianblurF32U8##interpolate##C1Kernel(                               \
    global const float* src, int src_offset, int rows, int cols,             \
    global const float* filter_kernel, int kernel_offset, int radius,        \
    int src_stride, global uchar* dst, int dst_stride, int dst_offset) {     \
...
#define GAUSSIANBLURU8F32C1KERNEL(interpolate)                               \
__kernel                                                                     \
void gaussianblurU8F32##interpolate##C1Kernel(                               \
    global const uchar* src, int src_offset, int rows, int cols,             \
    global const float* filter_kernel, int kernel_offset, int radius,        \
    int src_stride, global float* dst, int dst_stride, int dst_offset) {     \
  int element_x = get_global_id(0);                                          \
  int element_y = get_global_id(1);                                          \
  int group_x = get_group_id(0);                                             \
  int group_y = get_group_id(1);                                             \
...
#define GAUSSIANBLURF32F32C1KERNEL(interpolate)                              \
__kernel                                                                     \
void gaussianblurF32F32##interpolate##C1Kernel(                              \
    global const float* src, int src_offset, int rows, int cols,             \
    global const float* filter_kernel, int kernel_offset, int radius,        \
    int src_stride, global float* dst, int dst_stride, int dst_offset) {     \
  int element_x = get_global_id(0);                                          \
...
#define GAUSSIANBLURU8F32C3KERNEL(interpolate)                               \
__kernel                                                                     \
void gaussianblurU8F32##interpolate##C3Kernel(                               \
    global const uchar* src, int src_offset, int rows, int cols,             \
    global const float* filter_kernel, int kernel_offset, int radius,        \
    int src_stride, global float* dst, int dst_stride, int dst_offset) {     \
  int element_x = get_global_id(0);                                          \
  ...
  for (int i = 0; i < min(remain_rows, 1); i++) {                            \
    ...
    if (isnt_border_block) {                                                 \
      ...
      ((float3*)input_value + i)[0] +=                                       \
          convert_float3(vload3(index_x, src)) *                             \
          filter_kernel[filter_kernel_index];                                \
    }                                                                        \
    else {                                                                   \
      for (int j = radius; j > 0; j--) {                                     \
        data_index = interpolate(cols, radius, index_x - j);                 \
        data_index0 = interpolate(cols, radius, index_x + j);                \
        ((float3*)input_value + i)[0] +=                                     \
      ...
      data_index = interpolate(cols, radius, index_x);                       \
      ((float3*)input_value + i)[0] +=                                       \
          convert_float3(vload3(data_index, src)) *                          \
          filter_kernel[filter_kernel_index];                                \
    }                                                                        \
...
#define GAUSSIANBLURF32U8C3KERNEL(interpolate)                               \
__kernel                                                                     \
void gaussianblurF32U8##interpolate##C3Kernel(                               \
    global const float* src, int src_offset, int rows, int cols,             \
    global const float* filter_kernel, int kernel_offset, int radius,        \
    int src_stride, global uchar* dst, int dst_stride, int dst_offset) {     \
  int element_x = get_global_id(0);                                          \
...
#define GAUSSIANBLURF32F32C3KERNEL(interpolate)                              \
__kernel                                                                     \
void gaussianblurF32F32##interpolate##C3Kernel(                              \
    global const float* src, int src_offset, int rows, int cols,             \
    global const float* filter_kernel, int kernel_offset, int radius,        \
    int src_stride, global float* dst, int dst_stride, int dst_offset) {     \
  int element_x = get_global_id(0);                                          \
...
#define GAUSSIANBLURU8F32C4KERNEL(interpolate)                               \
__kernel                                                                     \
void gaussianblurU8F32##interpolate##C4Kernel(                               \
    global const uchar* src, int src_offset, int rows, int cols,             \
    global const float* filter_kernel, int kernel_offset, int radius,        \
    int src_stride, global float* dst, int dst_stride, int dst_offset) {     \
  int element_x = get_global_id(0);                                          \
  ...
  for (int i = 0; i < min(remain_rows, 1); i++) {                            \
    ...
    if (isnt_border_block) {                                                 \
      ...
      ((float4*)input_value + i)[0] +=                                       \
          convert_float4(vload4(index_x, src)) *                             \
          filter_kernel[filter_kernel_index];                                \
    }                                                                        \
    else {                                                                   \
      for (int j = radius; j > 0; j--) {                                     \
...
#define GAUSSIANBLURF32U8C4KERNEL(interpolate)                               \
__kernel                                                                     \
void gaussianblurF32U8##interpolate##C4Kernel(                               \
    global const float* src, int src_offset, int rows, int cols,             \
    global const float* filter_kernel, int kernel_offset, int radius,        \
    int src_stride, global uchar* dst, int dst_stride, int dst_offset) {     \
  int element_x = get_global_id(0);                                          \
...
#define GAUSSIANBLURF32F32C4KERNEL(interpolate)                              \
__kernel                                                                     \
void gaussianblurF32F32##interpolate##C4Kernel(                              \
    global const float* src, int src_offset, int rows, int cols,             \
    global const float* filter_kernel, int kernel_offset, int radius,        \
    int src_stride, global float* dst, int dst_stride, int dst_offset) {     \
  int element_x = get_global_id(0);                                          \
...
#if defined(GAUSSIANBLUR_interpolateReplicateBorderU8C1) || defined(ALL_KERNELS)
GAUSSIANBLURF32U8C1KERNEL(interpolateReplicateBorder)
GAUSSIANBLURU8F32C1KERNEL(interpolateReplicateBorder)
#endif

#if defined(GAUSSIANBLUR_interpolateReplicateBorderF32C1) || defined(ALL_KERNELS)
GAUSSIANBLURF32F32C1KERNEL(interpolateReplicateBorder)
#endif

#if defined(GAUSSIANBLUR_interpolateReplicateBorderU8C3) || defined(ALL_KERNELS)
GAUSSIANBLURU8F32C3KERNEL(interpolateReplicateBorder)
GAUSSIANBLURF32U8C3KERNEL(interpolateReplicateBorder)
#endif
#if defined(GAUSSIANBLUR_interpolateReplicateBorderF32C3) || defined(ALL_KERNELS)
GAUSSIANBLURF32F32C3KERNEL(interpolateReplicateBorder)
#endif

#if defined(GAUSSIANBLUR_interpolateReplicateBorderU8C4) || defined(ALL_KERNELS)
GAUSSIANBLURU8F32C4KERNEL(interpolateReplicateBorder)
GAUSSIANBLURF32U8C4KERNEL(interpolateReplicateBorder)
#endif
#if defined(GAUSSIANBLUR_interpolateReplicateBorderF32C4) || defined(ALL_KERNELS)
GAUSSIANBLURF32F32C4KERNEL(interpolateReplicateBorder)
#endif



#if defined(GAUSSIANBLUR_interpolateReflectBorderU8C1) || defined(ALL_KERNELS)
GAUSSIANBLURF32U8C1KERNEL(interpolateReflectBorder)
GAUSSIANBLURU8F32C1KERNEL(interpolateReflectBorder)
#endif

#if defined(GAUSSIANBLUR_interpolateReflectBorderF32C1) || defined(ALL_KERNELS)
GAUSSIANBLURF32F32C1KERNEL(interpolateReflectBorder)
#endif

#if defined(GAUSSIANBLUR_interpolateReflectBorderU8C3) || defined(ALL_KERNELS)
GAUSSIANBLURU8F32C3KERNEL(interpolateReflectBorder)
GAUSSIANBLURF32U8C3KERNEL(interpolateReflectBorder)
#endif
#if defined(GAUSSIANBLUR_interpolateReflectBorderF32C3) || defined(ALL_KERNELS)
GAUSSIANBLURF32F32C3KERNEL(interpolateReflectBorder)
#endif

#if defined(GAUSSIANBLUR_interpolateReflectBorderU8C4) || defined(ALL_KERNELS)
GAUSSIANBLURU8F32C4KERNEL(interpolateReflectBorder)
GAUSSIANBLURF32U8C4KERNEL(interpolateReflectBorder)
#endif
#if defined(GAUSSIANBLUR_interpolateReflectBorderF32C4) || defined(ALL_KERNELS)
GAUSSIANBLURF32F32C4KERNEL(interpolateReflectBorder)
#endif



#if defined(GAUSSIANBLUR_interpolateReflect101BorderU8C1) || defined(ALL_KERNELS)
GAUSSIANBLURF32U8C1KERNEL(interpolateReflect101Border)
GAUSSIANBLURU8F32C1KERNEL(interpolateReflect101Border)
#endif

#if defined(GAUSSIANBLUR_interpolateReflect101BorderF32C1) || defined(ALL_KERNELS)
GAUSSIANBLURF32F32C1KERNEL(interpolateReflect101Border)
#endif

#if defined(GAUSSIANBLUR_interpolateReflect101BorderU8C3) || defined(ALL_KERNELS)
GAUSSIANBLURU8F32C3KERNEL(interpolateReflect101Border)
GAUSSIANBLURF32U8C3KERNEL(interpolateReflect101Border)
#endif
#if defined(GAUSSIANBLUR_interpolateReflect101BorderF32C3) || defined(ALL_KERNELS)
GAUSSIANBLURF32F32C3KERNEL(interpolateReflect101Border)
#endif

#if defined(GAUSSIANBLUR_interpolateReflect101BorderU8C4) || defined(ALL_KERNELS)
GAUSSIANBLURU8F32C4KERNEL(interpolateReflect101Border)
GAUSSIANBLURF32U8C4KERNEL(interpolateReflect101Border)
#endif
#if defined(GAUSSIANBLUR_interpolateReflect101BorderF32C4) || defined(ALL_KERNELS)
GAUSSIANBLURF32F32C4KERNEL(interpolateReflect101Border)
#endif
