...
#define SEPFILTER2DF32U8C1KERNEL(interpolate)                                \
__kernel                                                                     \
void sepfilter2dF32U8##interpolate##C1Kernel(                                \
    global const float* src, int src_offset, int rows, int cols,             \
    global const float* filter_kernel, int radius, int src_stride,           \
    global uchar* dst, int dst_stride, int is_symmetric, float delta,        \
    int dst_offset) {                                                        \
    ...
    if (isnt_border_block) {                                                 \
      ...
      for (int j = bottom; j <= top; j++) {                                  \
        ((float4*)input_value + i)[0] +=                                     \
            convert_float4(vload4(0, src_temp)) *                            \
...
#define SEPFILTER2DU8F32C1KERNEL(interpolate)                                \
__kernel                                                                     \
void sepfilter2dU8F32##interpolate##C1Kernel(                                \
    global const uchar* src, int src_offset, int rows, int cols,             \
    global const float* filter_kernel, int radius, int src_stride,           \
    global float* dst, int dst_stride, int is_symmetric, float delta,        \
    int dst_offset) {                                                        \
  int element_x = get_global_id(0);                                          \
  int element_y = get_global_id(1);                                          \
  int group_x = get_group_id(0);                                             \
...
#define SEPFILTER2DF32F32C1KERNEL(interpolate)                               \
__kernel                                                                     \
void sepfilter2dF32F32##interpolate##C1Kernel(                               \
    global const float* src, int src_offset, int rows, int cols,             \
    global const float* filter_kernel, int radius, int src_stride,           \
    global float* dst, int dst_stride, int is_symmetric, float delta,        \
    int dst_offset) {                                                        \
...
#define SEPFILTER2DU8F32C3KERNEL(interpolate)                                \
__kernel                                                                     \
void sepfilter2dU8F32##interpolate##C3Kernel(                                \
    global const uchar* src, int src_offset, int rows, int cols,             \
    global const float* filter_kernel, int radius, int src_stride,           \
    global float* dst, int dst_stride, int is_symmetric, float delta,        \
    int dst_offset) {                                                        \
...
#define SEPFILTER2DF32U8C3KERNEL(interpolate)                                \
__kernel                                                                     \
void sepfilter2dF32U8##interpolate##C3Kernel(                                \
    global const float* src, int src_offset, int rows, int cols,             \
    global const float* filter_kernel, int radius, int src_stride,           \
    global uchar* dst, int dst_stride, int is_symmetric, float delta,        \
    int dst_offset) {                                                        \
...
#define SEPFILTER2DF32F32C3KERNEL(interpolate)                               \
__kernel                                                                     \
void sepfilter2dF32F32##interpolate##C3Kernel(                               \
    global const float* src, int src_offset, int rows, int cols,             \
    global const float* filter_kernel, int radius, int src_stride,           \
    global float* dst, int dst_stride, int is_symmetric, float delta,        \
    int dst_offset) {                                                        \
...
#define SEPFILTER2DU8F32C4KERNEL(interpolate)                                \
__kernel                                                                     \
void sepfilter2dU8F32##interpolate##C4Kernel(                                \
    global const uchar* src, int src_offset, int rows, int cols,             \
    global const float* filter_kernel, int radius, int src_stride,           \
    global float* dst, int dst_stride, int is_symmetric, float delta,        \
    int dst_offset) {                                                        \
...
#define SEPFILTER2DF32U8C4KERNEL(interpolate)                                \
__kernel                                                                     \
void sepfilter2dF32U8##interpolate##C4Kernel(                                \
    global const float* src, int src_offset, int rows, int cols,             \
    global const float* filter_kernel, int radius, int src_stride,           \
    global uchar* dst, int dst_stride, int is_symmetric, float delta,        \
    int dst_offset) {                                                        \
...
#define SEPFILTER2DF32F32C4KERNEL(interpolate)                               \
__kernel                                                                     \
void sepfilter2dF32F32##interpolate##C4Kernel(                               \
    global const float* src, int src_offset, int rows, int cols,             \
    global const float* filter_kernel, int radius, int src_stride,           \
    global float* dst, int dst_stride, int is_symmetric, float delta,        \
    int dst_offset) {                                                        \
...
#if defined(SEPFILTER2DinterpolateReplicateBorderU8C1) || defined(ALL_KERNELS)
SEPFILTER2DU8F32C1KERNEL(interpolateReplicateBorder)
SEPFILTER2DF32U8C1KERNEL(interpolateReplicateBorder)
#endif
#if defined(SEPFILTER2DinterpolateReplicateBorderF32C1) || defined(ALL_KERNELS)
SEPFILTER2DF32F32C1KERNEL(interpolateReplicateBorder)
#endif

#if defined(SEPFILTER2DinterpolateReflectBorderU8C1) || defined(ALL_KERNELS)
SEPFILTER2DU8F32C1KERNEL(interpolateReflectBorder)
SEPFILTER2DF32U8C1KERNEL(interpolateReflectBorder)
#endif
#if defined(SEPFILTER2DinterpolateReflectBorderF32C1) || defined(ALL_KERNELS)
SEPFILTER2DF32F32C1KERNEL(interpolateReflectBorder)
#endif

#if defined(SEPFILTER2DinterpolateReflect101BorderU8C1) || defined(ALL_KERNELS)
SEPFILTER2DU8F32C1KERNEL(interpolateReflect101Border)
SEPFILTER2DF32U8C1KERNEL(interpolateReflect101Border)
#endif
#if defined(SEPFILTER2DinterpolateReflect101BorderF32C1) || defined(ALL_KERNELS)
SEPFILTER2DF32F32C1KERNEL(interpolateReflect101Border)
#endif



#if defined(SEPFILTER2DinterpolateReplicateBorderU8C3) || defined(ALL_KERNELS)
SEPFILTER2DU8F32C3KERNEL(interpolateReplicateBorder)
SEPFILTER2DF32U8C3KERNEL(interpolateReplicateBorder)
#endif
#if defined(SEPFILTER2DinterpolateReplicateBorderF32C3) || defined(ALL_KERNELS)
SEPFILTER2DF32F32C3KERNEL(interpolateReplicateBorder)
#endif
#if defined(SEPFILTER2DinterpolateReflectBorderU8C3) || defined(ALL_KERNELS)
SEPFILTER2DU8F32C3KERNEL(interpolateReflectBorder)
SEPFILTER2DF32U8C3KERNEL(interpolateReflectBorder)
#endif
#if defined(SEPFILTER2DinterpolateReflectBorderF32C3) || defined(ALL_KERNELS)
SEPFILTER2DF32F32C3KERNEL(interpolateReflectBorder)
#endif
#if defined(SEPFILTER2DinterpolateReflect101BorderU8C3) || defined(ALL_KERNELS)
SEPFILTER2DU8F32C3KERNEL(interpolateReflect101Border)
SEPFILTER2DF32U8C3KERNEL(interpolateReflect101Border)
#endif
#if defined(SEPFILTER2DinterpolateReflect101BorderF32C3) || defined(ALL_KERNELS)
SEPFILTER2DF32F32C3KERNEL(interpolateReflect101Border)
#endif


#if defined(SEPFILTER2DinterpolateReplicateBorderU8C4) || defined(ALL_KERNELS)
SEPFILTER2DU8F32C4KERNEL(interpolateReplicateBorder)
SEPFILTER2DF32U8C4KERNEL(interpolateReplicateBorder)
#endif
#if defined(SEPFILTER2DinterpolateReplicateBorderF32C4) || defined(ALL_KERNELS)
SEPFILTER2DF32F32C4KERNEL(interpolateReplicateBorder)
#endif
#if defined(SEPFILTER2DinterpolateReflectBorderU8C4) || defined(ALL_KERNELS)
SEPFILTER2DU8F32C4KERNEL(interpolateReflectBorder)
SEPFILTER2DF32U8C4KERNEL(interpolateReflectBorder)
#endif
#if defined(SEPFILTER2DinterpolateReflectBorderF32C4) || defined(ALL_KERNELS)
SEPFILTER2DF32F32C4KERNEL(interpolateReflectBorder)
#endif
#if defined(SEPFILTER2DinterpolateReflect101BorderU8C4) || defined(ALL_KERNELS)
SEPFILTER2DU8F32C4KERNEL(interpolateReflect101Border)
SEPFILTER2DF32U8C4KERNEL(interpolateReflect101Border)
#endif
#if defined(SEPFILTER2DinterpolateReflect101BorderF32C4) || defined(ALL_KERNELS)
SEPFILTER2DF32F32C4KERNEL(interpolateReflect101Border)
#endif
