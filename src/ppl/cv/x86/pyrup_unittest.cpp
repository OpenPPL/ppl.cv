...
class PyrUp : public ::testing::TestWithParam<std::tuple<Size, Size>> {
public:
    using PyrUpParam = std::tuple<Size, Size>;
    PyrUp()
    {
    }
    ...
    void apply(const PyrUpParam &param) {
        ...
        ::cv::pyrUp(iMat, oMat, ::cv::Size(osize.width, osize.height));
        checkResult<T, c>(dst.get(), dst_opencv.get(), osize.height, osize.width, osize.width * c, osize.width * c, 1e-3);
    }
};

#define R(name, t, c)\
    using name = PyrUp<t, c>;\
    TEST_P(name, t ## c)\
    {\
        this->apply(GetParam());\
    }\
    INSTANTIATE_TEST_CASE_P(standard, name,\
        ::testing::Combine(::testing::Values(Size{3, 4}),\
                           ::testing::Values(Size{6, 8})));

R(PyrUp_x86_f32c1, float, 1)
R(PyrUp_x86_f32c3, float, 3)
R(PyrUp_x86_f32c4, float, 4)

R(PyrUp_x86_u8c1, uint8_t, 1)
R(PyrUp_x86_u8c3, uint8_t, 3)
R(PyrUp_x86_u8c4, uint8_t, 4)
