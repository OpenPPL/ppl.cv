...
void Color2YUV420MultiPlaneTest(int32_t height, int32_t width) {
    ...
    cv::Mat dstMat(3 * height / 2, width, CV_MAKETYPE(cv::DataType<uint8_t>::depth, 1), dst_ref.get());
    if (mode == RGB2I420_MODE) { 
        ppl::cv::x86::RGB2I420<uint8_t>(height, width, width * 3, src.get(),
                                            width, dst.get(),
                                            width / 2, dst.get() + height * width,
                                            width / 2, dst.get() + height * width + (height / 2) * (width / 2));
        cv::cvtColor(srcMat, dstMat,cv::COLOR_RGB2YUV_I420);
    } else if (mode == BGR2I420_MODE) { 
        ppl::cv::x86::BGR2I420<uint8_t>(height, width, width * 3, src.get(),
                                            width, dst.get(),
                                            width / 2, dst.get() + height * width,
                                            width / 2, dst.get() + height * width + (height / 2) * (width / 2));
        cv::cvtColor(srcMat, dstMat,cv::COLOR_BGR2YUV_I420);
...
enum ColorAlpha2YUV420Mode {RGBA2I420_MODE, BGRA2I420_MODE, RGBA2YV12_MODE, BGRA2YV12_MODE};
...
void ColorAlpha2YUV420Test(int32_t height, int32_t width) {
    ...
    if (mode == RGBA2I420_MODE) { 
        ppl::cv::x86::RGBA2I420<uint8_t>(height, width, width * 4, src.get(), width, dst.get());
        cv::cvtColor(srcMat, dstMat,cv::COLOR_RGBA2YUV_I420);
    } else if (mode == RGBA2YV12_MODE) {
        ppl::cv::x86::RGBA2YV12<uint8_t>(height, width, width * 4, src.get(), width, dst.get());
        cv::cvtColor(srcMat, dstMat,cv::COLOR_RGBA2YUV_YV12);
    } else if (mode == BGRA2I420_MODE) { 
        ppl::cv::x86::BGRA2I420<uint8_t>(height, width, width * 4, src.get(), width, dst.get());
        cv::cvtColor(srcMat, dstMat,cv::COLOR_BGRA2YUV_I420);
    } else if (mode == BGRA2YV12_MODE) {
        ppl::cv::x86::BGRA2YV12<uint8_t>(height, width, width * 4, src.get(), width, dst.get());
...
void ColorAlpha2YUV420MultiPlaneTest(int32_t height, int32_t width) {
    ...
    cv::Mat dstMat(3 * height / 2, width, CV_MAKETYPE(cv::DataType<uint8_t>::depth, 1), dst_ref.get());
    if (mode == RGBA2I420_MODE) { 
        ppl::cv::x86::RGBA2I420<uint8_t>(height, width, width * 4, src.get(),
                                                width, dst.get(),
                                                width / 2, dst.get() + height * width,
                                                width / 2, dst.get() + height * width + (height / 2) * (width / 2));
        cv::cvtColor(srcMat, dstMat,cv::COLOR_RGBA2YUV_I420);
    } else if (mode == BGRA2I420_MODE) { 
        ppl::cv::x86::BGRA2I420<uint8_t>(height, width, width * 4, src.get(),
                                                width, dst.get(),
                                                width / 2, dst.get() + height * width,
                                                width / 2, dst.get() + height * width + (height / 2) * (width / 2));
        cv::cvtColor(srcMat, dstMat,cv::COLOR_BGRA2YUV_I420);
...
void YUV4202ColorTest(int32_t height, int32_t width) {
    ...
    if (mode == I4202RGB_MODE) { 
        ppl::cv::x86::I4202RGB<uint8_t>(height, width, width, src.get(), 3 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2RGB_I420);
    } else if (mode == I4202BGR_MODE) {
        ppl::cv::x86::I4202BGR<uint8_t>(height, width, width, src.get(), 3 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2BGR_I420);
    } else if (mode == YV122RGB_MODE) { 
        ppl::cv::x86::YV122RGB<uint8_t>(height, width, width, src.get(), 3 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2RGB_YV12);
    } else if (mode == YV122BGR_MODE) {
        ppl::cv::x86::YV122BGR<uint8_t>(height, width, width, src.get(), 3 * width, dst.get());
...
void YUV4202ColorMultiPlaneTest(int32_t height, int32_t width) {
    ...
    cv::Mat dstMat(height, width, CV_MAKETYPE(cv::DataType<uint8_t>::depth, 3), dst_ref.get());
    if (mode == I4202RGB_MODE) { 
        ppl::cv::x86::I4202RGB<uint8_t>(height, width,
                                            width, src.get(),
                                            width / 2, src.get() + height * width,
                                            width / 2, src.get() + height * width + (height / 2) * (width / 2),
                                            3 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2RGB_I420);
    } else if (mode == I4202BGR_MODE) {
        ppl::cv::x86::I4202BGR<uint8_t>(height, width,
                                            width, src.get(),
                                            width / 2, src.get() + height * width,
                                            width / 2, src.get() + height * width + (height / 2) * (width / 2),
                                            3 * width, dst.get());
...
enum YUV4202ColorAlphaMode {I4202RGBA_MODE, I4202BGRA_MODE, YV122RGBA_MODE, YV122BGRA_MODE};
...
void YUV4202ColorAlphaTest(int32_t height, int32_t width) {
    ...
    if (mode == I4202RGBA_MODE) { 
        ppl::cv::x86::I4202RGBA<uint8_t>(height, width, width, src.get(), 4 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2RGBA_I420);
    } else if (mode == I4202BGRA_MODE) {
        ppl::cv::x86::I4202BGRA<uint8_t>(height, width, width, src.get(), 4 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2BGRA_I420);
    } else if (mode == YV122RGBA_MODE) { 
        ppl::cv::x86::YV122RGBA<uint8_t>(height, width, width, src.get(), 4 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2RGBA_YV12);
    } else if (mode == YV122BGRA_MODE) {
        ppl::cv::x86::YV122BGRA<uint8_t>(height, width, width, src.get(), 4 * width, dst.get());
...
void YUV4202ColorAlphaMultiPlaneTest(int32_t height, int32_t width) {
    ...
    cv::Mat dstMat(height, width, CV_MAKETYPE(cv::DataType<uint8_t>::depth, 4), dst_ref.get());
    if (mode == I4202RGBA_MODE) { 
        ppl::cv::x86::I4202RGBA<uint8_t>(height, width,
                                            width, src.get(),
                                            width / 2, src.get() + height * width,
                                            width / 2, src.get() + height * width + (height / 2) * (width / 2),
                                            4 * width, dst.get());
        ...
        ppl::cv::x86::I4202BGRA<uint8_t>(height, width,
                                            width, src.get(),
                                            width / 2, src.get() + height * width,
                                            width / 2, src.get() + height * width + (height / 2) * (width / 2),
                                            4 * width, dst.get());
