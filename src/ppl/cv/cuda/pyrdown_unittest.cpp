...
class PplCvCudaPyrDownTest : public ::testing::TestWithParam<Parameters> {
 public:
  PplCvCudaPyrDownTest() {
...
template <typename T, int channels>
bool PplCvCudaPyrDownTest<T, channels>::apply() {
  ...
  PyrDown<T, channels>(0, gpu_src.rows, gpu_src.cols, gpu_src.step / sizeof(T),
                       (T*)gpu_src.data, gpu_dst.step / sizeof(T),
                       (T*)gpu_dst.data, border_type);
  gpu_dst.download(dst);

  PyrDown<T, channels>(0, src_height, src_width, src_width * channels,
                       gpu_input, dst_width * channels, gpu_output,
                       border_type);
...
using PplCvCudaPyrDownTest ## T ## channels =                                  \
        PplCvCudaPyrDownTest<T, channels>;                                     \
TEST_P(PplCvCudaPyrDownTest ## T ## channels, Standard) {                      \
...
INSTANTIATE_TEST_CASE_P(IsEqual, PplCvCudaPyrDownTest ## T ## channels,        \
  ...
  [](const testing::TestParamInfo<                                             \
       PplCvCudaPyrDownTest ## T ## channels::ParamType>& info) {              \
