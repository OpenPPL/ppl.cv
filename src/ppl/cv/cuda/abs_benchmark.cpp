...
void BM_Abs_ppl_cuda(benchmark::State &state) {
...
BENCHMARK_TEMPLATE(BM_Abs_opencv_cuda, schar, channels)->                      \
                   Args({width, height})->UseManualTime()->Iterations(10);     \
BENCHMARK_TEMPLATE(BM_Abs_ppl_cuda, schar, channels)->                         \
                   Args({width, height})->UseManualTime()->Iterations(10);     \
BENCHMARK_TEMPLATE(BM_Abs_opencv_cuda, float, channels)->                      \
                   Args({width, height})->UseManualTime()->Iterations(10);     \
BENCHMARK_TEMPLATE(BM_Abs_ppl_cuda, float, channels)->                         \
                   Args({width, height})->UseManualTime()->Iterations(10);
...
BENCHMARK_TEMPLATE(BM_Abs_opencv_cuda, type, c1)->Args({640, 480})->           \
                   UseManualTime()->Iterations(10);                            \
BENCHMARK_TEMPLATE(BM_Abs_opencv_cuda, type, c3)->Args({640, 480})->           \
                   UseManualTime()->Iterations(10);                            \
BENCHMARK_TEMPLATE(BM_Abs_opencv_cuda, type, c4)->Args({640, 480})->           \
                   UseManualTime()->Iterations(10);

#define RUN_PPL_CV_TYPE_FUNCTIONS(type)                                        \
BENCHMARK_TEMPLATE(BM_Abs_ppl_cuda, type, c1)->Args({640, 480})->              \
                   UseManualTime()->Iterations(10);                            \
BENCHMARK_TEMPLATE(BM_Abs_ppl_cuda, type, c3)->Args({640, 480})->              \
                   UseManualTime()->Iterations(10);                            \
BENCHMARK_TEMPLATE(BM_Abs_ppl_cuda, type, c4)->Args({640, 480})->              \
                   UseManualTime()->Iterations(10);

RUN_OPENCV_TYPE_FUNCTIONS(schar)
RUN_OPENCV_TYPE_FUNCTIONS(float)

RUN_PPL_CV_TYPE_FUNCTIONS(schar)
