...
__global__
void affineLinearTextureKernel(const uchar* src, int src_rows, int src_cols,
                               int channels, int src_stride, float coeffe0,
                               float coeffe1, float coeffe2, float coeffe3,
                               float coeffe4, float coeffe5, uchar* dst,
...
__global__
void affineLinearTextureKernel(const float* src, int src_rows, int src_cols,
                               int channels, int src_stride, float coeffe0,
                               float coeffe1, float coeffe2, float coeffe3,
                               float coeffe4, float coeffe5, float* dst,
                               int dst_rows, int dst_cols, int dst_stride,
...
__global__
void warpAffineNearestPointKernel(const T* src, int src_rows, int src_cols,
                                  int channels, int src_stride, float coeffe0,
                                  float coeffe1, float coeffe2, float coeffe3,
                                  float coeffe4, float coeffe5, T* dst,
                                  int dst_rows, int dst_cols, int dst_stride,
...
RetCode warpAffine(const uchar* src, int src_rows, int src_cols, int channels,
  ...
  if (interpolation == INTERPOLATION_TYPE_LINEAR) {
    if (channels != 3 && (src_stride & (TEXTURE_ALIGNMENT - 1)) == 0) {
      ...
      affineLinearTextureKernel<<<grid, block, 0, stream>>>(src, src_rows, 
          src_cols, channels, src_stride, affine_matrix[0], affine_matrix[1],
          affine_matrix[2], affine_matrix[3], affine_matrix[4], 
          affine_matrix[5], dst, dst_rows, dst_cols, dst_stride, border_type, 
          border_value);             
    }
    else {
      warpAffineLinearKernel<<<grid, block, 0, stream>>>(src, src_rows, 
          src_cols, channels, src_stride, affine_matrix[0], affine_matrix[1],
          affine_matrix[2], affine_matrix[3], affine_matrix[4], 
          affine_matrix[5], dst, dst_rows, dst_cols, dst_stride, border_type, 
          border_value);      
    ...
    if (channels == 1) {
      warpAffineNearestPointKernel<uchar, uchar><<<grid, block, 0, stream>>>(
          ...
          affine_matrix[4], affine_matrix[5], dst, dst_rows, dst_cols,
          dst_stride, border_type, border_value);
    }
    else if (channels == 3) {
      uchar3 border_value1 = make_uchar3(border_value, border_value,
                                         border_value);
      warpAffineNearestPointKernel<uchar, uchar3><<<grid, block, 0, stream>>>(
          src, src_rows, src_cols, channels, src_stride, affine_matrix[0],
          affine_matrix[1], affine_matrix[2], affine_matrix[3],
          affine_matrix[4], affine_matrix[5], dst, dst_rows, dst_cols,
      ...
      uchar4 border_value1 = make_uchar4(border_value, border_value,
                                         border_value, border_value);
      warpAffineNearestPointKernel<uchar, uchar4><<<grid, block, 0, stream>>>(
          src, src_rows, src_cols, channels, src_stride, affine_matrix[0],
          affine_matrix[1], affine_matrix[2], affine_matrix[3],
          affine_matrix[4], affine_matrix[5], dst, dst_rows, dst_cols,
          dst_stride, border_type, border_value1);
...
RetCode warpAffine(const float* src, int src_rows, int src_cols, int channels,
  ...
  if (interpolation == INTERPOLATION_TYPE_LINEAR) {
    ...
    if (channels == 1) {
      ...
      float3 border_value1 = make_float3(border_value, border_value,
                                         border_value);
      warpAffineNearestPointKernel<float, float3><<<grid, block, 0, stream>>>(
          src, src_rows, src_cols, channels, src_stride, affine_matrix[0],
          affine_matrix[1], affine_matrix[2], affine_matrix[3],
          affine_matrix[4], affine_matrix[5], dst, dst_rows, dst_cols,
          dst_stride, border_type, border_value1);
      ...
      float4 border_value1 = make_float4(border_value, border_value,
                                         border_value, border_value);
      warpAffineNearestPointKernel<float, float4><<<grid, block, 0, stream>>>(
          src, src_rows, src_cols, channels, src_stride, affine_matrix[0],
          affine_matrix[1], affine_matrix[2], affine_matrix[3],
          affine_matrix[4], affine_matrix[5], dst, dst_rows, dst_cols,
          dst_stride, border_type, border_value1);
