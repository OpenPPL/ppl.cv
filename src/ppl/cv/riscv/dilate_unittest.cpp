...
public:
    ...
    void apply(const DilateParam &param)
    {
        ...
        ppl::cv::debug::randomFill<uchar>(kernel.get(), kernel_size * kernel_size, 0, 1);
        int32_t num_kernel_elem = 0;
        for (int32_t i = 0; i < kernel_size * kernel_size; ++i) {
            num_kernel_elem += (bool)kernel.get()[i];
        }
        while (!num_kernel_elem) {
            ppl::cv::debug::randomFill<uchar>(kernel.get(), kernel_size * kernel_size, 0, 1);
            num_kernel_elem = 0;
            for (int32_t i = 0; i < kernel_size * kernel_size; ++i) {
                num_kernel_elem += (bool)kernel.get()[i];
...
#define R1(name, t, c, diff)     \
    ...
    INSTANTIATE_TEST_CASE_P(standard, name, ::testing::Combine(::testing::Values(3, 4, 5, 10, 240, 241, 242, 243, 244, 245, 246, 247, 248, 249, 250, 251, 252, 253, 254), ::testing::Values(10), ::testing::Values(3, 5), ::testing::Values(ppl::cv::BORDER_CONSTANT, ppl::cv::BORDER_REFLECT, ppl::cv::BORDER_REFLECT101, ppl::cv::BORDER_REPLICATE), ::testing::Values(diff)));
