...
void BM_getAffineTransform_ppl_riscv(benchmark::State &state)
{
    double src[6], dst[6];
    double mat[6];
    ...
    dst[5] = 91;
    for (auto _ : state) {
        ppl::cv::riscv::GetAffineTransform(src, dst, mat, inverse_mat);
    }
    state.SetItemsProcessed(state.iterations() * 1);
}

BENCHMARK(BM_getAffineTransform_ppl_riscv);

#ifdef PPLCV_BENCHMARK_OPENCV
void BM_getAffineTransform_opencv_riscv(benchmark::State &state)
{
    cv::Point2f srcTri[3];
    cv::Point2f dstTri[3];
...
BENCHMARK(BM_getAffineTransform_opencv_riscv);
