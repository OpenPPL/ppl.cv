...
void Color2NVTest(int32_t height, int32_t width)
{
    ...
    if (mode == RGB2NV12_MODE) {
        ...
        ppl::cv::aarch64::I4202NV12(height, width, width, i420_ptr, width / 2, i420_ptr + height * width, width / 2, i420_ptr + height * width + (height / 2) * (width / 2), width, dst_ref.get(), width, dst_ref.get() + height * width);
        ppl::cv::aarch64::RGB2NV12<uint8_t>(height, width, width * 3, src.get(), width, dst.get());
    } else if (mode == RGB2NV21_MODE) {
        cv::cvtColor(srcMat, dstMatI420, cv::COLOR_RGB2YUV_I420);
        ppl::cv::aarch64::RGB2NV21<uint8_t>(height, width, width * 3, src.get(), width, dst.get());
        ppl::cv::aarch64::I4202NV21(height, width, width, i420_ptr, width / 2, i420_ptr + height * width, width / 2, i420_ptr + height * width + (height / 2) * (width / 2), width, dst_ref.get(), width, dst_ref.get() + height * width);
    } else if (mode == BGR2NV12_MODE) {
        cv::cvtColor(srcMat, dstMatI420, cv::COLOR_BGR2YUV_I420);
        ppl::cv::aarch64::BGR2NV12<uint8_t>(height, width, width * 3, src.get(), width, dst.get());
        ppl::cv::aarch64::I4202NV12(height, width, width, i420_ptr, width / 2, i420_ptr + height * width, width / 2, i420_ptr + height * width + (height / 2) * (width / 2), width, dst_ref.get(), width, dst_ref.get() + height * width);
    } else if (mode == BGR2NV21_MODE) {
        cv::cvtColor(srcMat, dstMatI420, cv::COLOR_BGR2YUV_I420);
        ppl::cv::aarch64::BGR2NV21<uint8_t>(height, width, width * 3, src.get(), width, dst.get());
        ppl::cv::aarch64::I4202NV21(height, width, width, i420_ptr, width / 2, i420_ptr + height * width, width / 2, i420_ptr + height * width + (height / 2) * (width / 2), width, dst_ref.get(), width, dst_ref.get() + height * width);
    }
...
void Color2NVMultiPlaneTest(int32_t height, int32_t width)
{
    ...
    if (mode == RGB2NV12_MODE) {
        cv::cvtColor(srcMat, dstMatI420, cv::COLOR_RGB2YUV_I420);
        ppl::cv::aarch64::RGB2NV12<uint8_t>(height, width, width * 3, src.get(), width, dst.get(), width, dst.get() + height * width);
        ppl::cv::aarch64::I4202NV12(height, width, width, i420_ptr, width / 2, i420_ptr + height * width, width / 2, i420_ptr + height * width + (height / 2) * (width / 2), width, dst_ref.get(), width, dst_ref.get() + height * width);
    } else if (mode == RGB2NV21_MODE) {
        cv::cvtColor(srcMat, dstMatI420, cv::COLOR_RGB2YUV_I420);
        ppl::cv::aarch64::RGB2NV21<uint8_t>(height, width, width * 3, src.get(), width, dst.get(), width, dst.get() + height * width);
        ppl::cv::aarch64::I4202NV21(height, width, width, i420_ptr, width / 2, i420_ptr + height * width, width / 2, i420_ptr + height * width + (height / 2) * (width / 2), width, dst_ref.get(), width, dst_ref.get() + height * width);
    } else if (mode == BGR2NV12_MODE) {
        cv::cvtColor(srcMat, dstMatI420, cv::COLOR_BGR2YUV_I420);
        ppl::cv::aarch64::BGR2NV12<uint8_t>(height, width, width * 3, src.get(), width, dst.get(), width, dst.get() + height * width);
        ppl::cv::aarch64::I4202NV12(height, width, width, i420_ptr, width / 2, i420_ptr + height * width, width / 2, i420_ptr + height * width + (height / 2) * (width / 2), width, dst_ref.get(), width, dst_ref.get() + height * width);
    } else if (mode == BGR2NV21_MODE) {
        cv::cvtColor(srcMat, dstMatI420, cv::COLOR_BGR2YUV_I420);
        ppl::cv::aarch64::BGR2NV21<uint8_t>(height, width, width * 3, src.get(), width, dst.get(), width, dst.get() + height * width);
        ppl::cv::aarch64::I4202NV21(height, width, width, i420_ptr, width / 2, i420_ptr + height * width, width / 2, i420_ptr + height * width + (height / 2) * (width / 2), width, dst_ref.get(), width, dst_ref.get() + height * width);
    }
    checkResult<uint8_t, 1>(dst.get(), dst_ref.get(), 3 * height / 2, width, width, width, 1.01f);
}
...
void NV2ColorTest(int32_t height, int32_t width)
{
    ...
    if (mode == NV122RGB_MODE) {
        ppl::cv::aarch64::NV122RGB<uint8_t>(height, width, width, src.get(), 3 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2RGB_NV12);
    } else if (mode == NV122BGR_MODE) {
        ppl::cv::aarch64::NV122BGR<uint8_t>(height, width, width, src.get(), 3 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2BGR_NV12);
    } else if (mode == NV212RGB_MODE) {
        ppl::cv::aarch64::NV212RGB<uint8_t>(height, width, width, src.get(), 3 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2RGB_NV21);
    } else if (mode == NV212BGR_MODE) {
        ppl::cv::aarch64::NV212BGR<uint8_t>(height, width, width, src.get(), 3 * width, dst.get());
...
void NV2ColorMultiPlaneTest(int32_t height, int32_t width)
{
    ...
    cv::Mat dstMat(height, width, CV_MAKETYPE(cv::DataType<uint8_t>::depth, 3), dst_ref.get());
    if (mode == NV122RGB_MODE) {
        ppl::cv::aarch64::NV122RGB<uint8_t>(height, width, width, src.get(), width, src.get() + height * width, 3 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2RGB_NV12);
    } else if (mode == NV122BGR_MODE) {
        ppl::cv::aarch64::NV122BGR<uint8_t>(height, width, width, src.get(), width, src.get() + height * width, 3 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2BGR_NV12);
    } else if (mode == NV212RGB_MODE) {
        ppl::cv::aarch64::NV212RGB<uint8_t>(height, width, width, src.get(), width, src.get() + height * width, 3 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2RGB_NV21);
    } else if (mode == NV212BGR_MODE) {
        ppl::cv::aarch64::NV212BGR<uint8_t>(height, width, width, src.get(), width, src.get() + height * width, 3 * width, dst.get());
        cv::cvtColor(srcMat, dstMat, cv::COLOR_YUV2BGR_NV21);
    }
    checkResult<uint8_t, 3>(dst.get(), dst_ref.get(), height, width, 3 * width, 3 * width, 2.01f);
}

TEST(NV12_2_RGB, aarch64)
{
