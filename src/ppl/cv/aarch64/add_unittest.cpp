...
void AddTest(int32_t height, int32_t width)
{
    ...
    ppl::cv::aarch64::Add<T, nc>(height, width, width * nc, src0.get(), width * nc, src1.get(), width * nc, dst.get());
...
TEST(AddTest, aarch64)
{
    AddTest<float, 1>(640, 720);
    AddTest<float, 3>(640, 720);
    AddTest<float, 4>(640, 720);
    AddTest<uint8_t, 1>(640, 720);
    AddTest<uint8_t, 3>(640, 720);
    AddTest<uint8_t, 4>(640, 720);
}
