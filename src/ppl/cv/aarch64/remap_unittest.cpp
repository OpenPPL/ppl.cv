...
public:
    ...
    void apply(const RemapParam &param) {
        ...
        if(inter_linear == true) {
            ppl::cv::aarch64::RemapLinear<T, c>(size.inHeight, size.inWidth, size.inWidth * c, src.get(), 
                                            size.outHeight, size.outWidth, size.outWidth * c, dst.get(), 
                                            map_x.get(), map_y.get(), borderMode, (T)border_value);    
        } else {
            ppl::cv::aarch64::RemapNearestPoint<T, c>(size.inHeight, size.inWidth, size.inWidth * c, src.get(), 
                                            size.outHeight, size.outWidth, size.outWidth * c, dst.get(), 
                                            map_x.get(), map_y.get(), borderMode, (T)border_value);
        }
